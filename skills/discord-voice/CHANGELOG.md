...
### Added
- Response chunking for lower perceived TTS latency
  - Splits agent responses into sentences
  - Starts playing first sentence while subsequent ones are synthesized
  - Configurable via `responseChunking` option (enabled by default)
  - Smart sentence boundary detection with abbreviation handling
  - Merges very short sentences for smoother flow
  - Respects barge-in between chunks
